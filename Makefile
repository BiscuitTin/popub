...
install: all
	install -m0755 popub-local/popub-local "$(DESTDIR)$(PREFIX)/bin/popub-local"
	install -m0755 popub-relay/popub-relay "$(DESTDIR)$(PREFIX)/bin/popub-relay"
	$(MAKE) -C launchd install DESTDIR="$(DESTDIR)" PREFIX="$(PREFIX)"

uninstall:
	rm -f "$(PREFIX)/bin/popub-local" "$(DESTDIR)$(PREFIX)/bin/popub-relay"
	$(MAKE) -C launchd uninstall DESTDIR="$(DESTDIR)" PREFIX="$(PREFIX)"
